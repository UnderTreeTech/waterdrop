...
// This guards the fix where Resolver.Close() previously shut down the whole
// etcd client (taking server-side KeepAlive and NewMutex down with it).
func TestEtcdResolver(t *testing.T) {
	...
	// grpc 1.40: Target has an Endpoint field (no URL/Endpoint() method).
	tgt := resolver.Target{Endpoint: svcName}
	...
	// client must remain usable: deregister and reads still work.
	rs.Close()
